    ...
    public void applyTransformation(float interpolatedTime, Ring ring, boolean lastFrame) {
        ...
        if (this.mFinishing) {
            ...
            if (interpolatedTime < 0.5f) {
                ...
                endTrim = startTrim + (MATERIAL_INTERPOLATOR.getInterpolation(interpolatedTime / 0.5f) * 0.79f) + MIN_PROGRESS_ARC;
            } else {
                endTrim = ring.getStartingStartTrim() + 0.79f;
                startTrim = endTrim - (((1.0f - MATERIAL_INTERPOLATOR.getInterpolation((interpolatedTime - 0.5f) / 0.5f)) * 0.79f) + MIN_PROGRESS_ARC);
            }
            float groupRotation = GROUP_FULL_ROTATION * (this.mRotationCount + interpolatedTime);
            ring.setStartTrim(startTrim);
            ring.setEndTrim(endTrim);
            ring.setRotation(startingRotation + (RING_ROTATION * interpolatedTime));
