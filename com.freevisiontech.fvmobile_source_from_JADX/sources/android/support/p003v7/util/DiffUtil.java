...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f1162x - o2.f1162x;
            return cmpX == 0 ? o1.f1163y - o2.f1163y : cmpX;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!arrayList.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f1162x += range.oldListStart;
                snake.f1163y += range.newListStart;
                ...
                if (snake.reverse) {
                    left.oldListEnd = snake.f1162x;
                    left.newListEnd = snake.f1163y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f1162x - 1;
                    left.newListEnd = snake.f1163y;
                } else {
                    left.oldListEnd = snake.f1162x;
                    left.newListEnd = snake.f1163y - 1;
                }
                arrayList.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f1162x + snake.size;
                    right.newListStart = snake.f1163y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = snake.f1162x + snake.size + 1;
                    right.newListStart = snake.f1163y + snake.size;
                } else {
                    right.oldListStart = snake.f1162x + snake.size;
                    right.newListStart = snake.f1163y + snake.size + 1;
                }
                arrayList.add(right);
            } else {
        ...
        for (int d = 0; d <= dLimit; d++) {
            ...
            while (k <= d) {
                ...
                if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || forward[kOffset + k] < backward[kOffset + k]) {
                    ...
                    outSnake.f1162x = backward[kOffset + k];
                    outSnake.f1163y = outSnake.f1162x - k;
            ...
            while (k2 <= d) {
                ...
                if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                    ...
                    outSnake2.f1162x = backward[kOffset + backwardK];
                    outSnake2.f1163y = outSnake2.f1162x - backwardK;
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f1162x != 0 || firstSnake.f1163y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f1162x + snake.size;
                int endY = snake.f1163y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f1162x + j;
                    int newItemPos = snake.f1163y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f1162x + snakeSize;
                int endY = snake.f1163y + snakeSize;
                if (endX < posOld) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f1162x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f1162x + i, 1, this.mCallback.getChangePayload(snake.f1162x + i, snake.f1163y + i));
                    }
                }
                posOld = snake.f1162x;
                posNew = snake.f1163y;
