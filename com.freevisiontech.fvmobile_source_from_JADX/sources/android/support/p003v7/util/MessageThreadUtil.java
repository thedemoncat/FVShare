...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public ThreadUtil.MainThreadCallback<T> getMainThreadProxy(final ThreadUtil.MainThreadCallback<T> callback) {
        return new ThreadUtil.MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new Runnable() {
                public void run() {
                    SyncQueueItem msg = C18191.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public ThreadUtil.BackgroundCallback<T> getBackgroundProxy(final ThreadUtil.BackgroundCallback<T> callback) {
        return new ThreadUtil.BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new Runnable() {
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C18212.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C18212.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C18212.this.mQueue.removeMessages(2);
                                    C18212.this.mQueue.removeMessages(3);
                            ...
                            C18212.this.mBackgroundRunning.set(false);
                            return;
                        }
                    }
                }
    ...
    static class MessageQueue {
        ...
        public synchronized void sendMessageAtFrontOfQueue(SyncQueueItem item) {
            SyncQueueItem unused = item.next = this.mRoot;
            this.mRoot = item;
        }
        ...
        public synchronized void sendMessage(SyncQueueItem item) {
            if (this.mRoot == null) {
                ...
                SyncQueueItem unused = last.next = item;
            }
        }
        ...
        public synchronized void removeMessages(int what) {
            ...
            if (this.mRoot != null) {
                ...
                while (item2 != null) {
                    ...
                    if (item2.what == what) {
                        SyncQueueItem unused = prev.next = next;
                        item2.recycle();
                    } else {
                        prev = item2;
                    }
