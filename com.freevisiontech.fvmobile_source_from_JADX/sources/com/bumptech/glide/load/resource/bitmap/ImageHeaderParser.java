...
public class ImageHeaderParser {
    ...
    private static class StreamReader {
        ...
        /* renamed from: is */
        private final InputStream f1186is;
        ...
        public int getUInt16() throws IOException {
            return ((this.f1186is.read() << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (this.f1186is.read() & 255);
        }

        public short getUInt8() throws IOException {
            return (short) (this.f1186is.read() & 255);
        }
        ...
        public long skip(long total) throws IOException {
            ...
            long toSkip = total;
            while (toSkip > 0) {
                long skipped = this.f1186is.skip(toSkip);
                if (skipped > 0) {
                    toSkip -= skipped;
                } else if (this.f1186is.read() == -1) {
                    break;
                } else {
                    toSkip--;
                }
        ...
        public int read(byte[] buffer) throws IOException {
            ...
            while (toRead > 0) {
                int read = this.f1186is.read(buffer, buffer.length - toRead, toRead);
                if (read == -1) {
                    break;
                }
                toRead -= read;
