...
public final class IsoTypeReader {
    public static long readUInt32BE(ByteBuffer bb) {
        ...
        return (((long) readUInt8(bb)) << 24) + (((long) readUInt8(bb)) << 16) + (ch2 << 8) + (ch1 << 0);
    }

    public static long readUInt32(ByteBuffer bb) {
        long i = (long) bb.getInt();
        if (i < 0) {
            return i + IjkMediaMeta.AV_CH_WIDE_RIGHT;
        }
        return i;
    }
    ...
    public static double readFixedPoint1616(ByteBuffer bb) {
        ...
        bb.get(bytes);
        return ((double) ((((0 | ((bytes[0] << 24) & -16777216)) | ((bytes[1] << C0217dk.f724n) & 16711680)) | ((bytes[2] << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK)) | (bytes[3] & 255))) / 65536.0d;
    }

    public static double readFixedPoint0230(ByteBuffer bb) {
        byte[] bytes = new byte[4];
        bb.get(bytes);
        return ((double) ((((0 | ((bytes[0] << 24) & -16777216)) | ((bytes[1] << C0217dk.f724n) & 16711680)) | ((bytes[2] << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK)) | (bytes[3] & 255))) / 1.073741824E9d;
    }

    public static float readFixedPoint88(ByteBuffer bb) {
        byte[] bytes = new byte[2];
        bb.get(bytes);
        return ((float) ((short) ((bytes[1] & 255) | ((short) (((bytes[0] << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | 0))))) / 256.0f;
    }
