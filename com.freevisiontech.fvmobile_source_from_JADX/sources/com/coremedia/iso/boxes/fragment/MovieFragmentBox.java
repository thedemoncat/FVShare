...
public class MovieFragmentBox extends AbstractContainerBox {
    ...
    public List<Long> getSyncSamples(SampleDependencyTypeBox sdtp) {
        ...
        long i = 1;
        for (SampleDependencyTypeBox.Entry sampleEntry : sdtp.getEntries()) {
    ...
    public long[] getTrackNumbers() {
        ...
        for (int trackCounter = 0; trackCounter < trackBoxes.size(); trackCounter++) {
            trackNumbers[trackCounter] = trackBoxes.get(trackCounter).getTrackFragmentHeaderBox().getTrackId();
        }
        return trackNumbers;
    }
