...
public class FVTrackManager implements Runnable {
    private static final String TAG = "FVTrackManager";
    private static XMiscing xmisc = null;

    /* renamed from: YK */
    private int f1116YK = 0;
    private Point center;
    private Size dimensions;
    private boolean islosted = false;
    private boolean istrack = false;
    private FVTrackObserver listener = null;
    private FVCamera1Manager manager = null;
    /* access modifiers changed from: private */
    public ObjTracking objTracking = new ObjTracking(0);
    ...
    public FVTrackManager(FVCamera1Manager parent, FVTrackObserver observer, int cameraPreviewWidth, int cameraPreviewHeight) {
        this.manager = parent;
    ...
    public void stopTrack() {
        ...
        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
            public void run() {
                FVTrackManager.this.yuvatfromcamera.release();
                FVTrackManager.this.objTracking.clear();
            }
        }, 500);
    }
    ...
    public boolean update(byte[] frame) {
        ...
        } else if (this.tracktype == 2 && !this.objTracking.isTrackPreinited() && !this.objTracking.preinit(this.yuvatfromcamera, this.startboundingBox)) {
            return false;
        }
        if (!this.objTracking.isTrackInited()) {
            ...
            CameraUtils.LogV("HTrack-objTrackUpdate", "objTracking update 总耗时: " + (System.currentTimeMillis() - t1));
        }
        if (!result) {
            CameraUtils.LogV("HTrack-Rect", "=======目标跟踪丢失========");
            if (this.listener == null) {
                return result;
            }
    ...
    private void sendPoint2Gimbal() {
        if (((Integer) SPUtils.get(MyApplication.getInstance().getApplicationContext(), SharePrefConstant.CAMERA_LENS_MODE, Integer.valueOf(Constants.LENS_BLACK_MODE))).intValue() == 10101) {
            if (((int) this.center.f1125x) == 0 && ((int) this.center.f1126y) == 0) {
                ...
                int sendX = (int) ((this.center.f1125x - ((double) (this.dimensions.getWidth() / 2))) * ((double) this.f1116YK));
                int sendY = (int) ((this.center.f1126y - ((double) (this.dimensions.getHeight() / 2))) * ((double) this.f1116YK));
            ...
            int sendX2 = (int) ((this.center.f1125x - ((double) (this.dimensions.getWidth() / 2))) * ((double) this.f1116YK));
            int sendY2 = (int) ((this.center.f1126y - ((double) (this.dimensions.getHeight() / 2))) * ((double) this.f1116YK));
            if (com.freevisiontech.fvmobile.utility.CameraUtils.getCurrentPageIndex() == 0) {
    ...
    private void getPictrueSizeAndPercent() {
        ...
        Size size = this.manager.getPreviewResolution();
        this.dimensions = size;
        this.f1116YK = (int) (((double) (SupportMenu.USER_MASK / size.getWidth())) * 0.5d);
    }
