...
public class DTSTrackImpl extends AbstractTrack {
            ...
            while (!done) {
                ...
                if (sync == 2147385345) {
                    if (corePresent == 1) {
                        ...
                        switch (vernum) {
                            case 6:
                                int i2 = -(brb.readBits(4) + 16);
                                break;
                            case 7:
                                int i3 = -brb.readBits(4);
                                break;
                            default:
                                brb.readBits(4);
                                break;
                    ...
                    extPresent = 1;
                    BitReaderBuffer brb2 = new BitReaderBuffer(bb);
                    brb2.readBits(8);
                    brb2.readBits(2);
                    ...
                    int nuExtSSHeaderSize = brb2.readBits(nuBits4Header) + 1;
    ...
    private int getSampleRate(int sfreq) throws IOException {
        switch (sfreq) {
            ...
            case 12:
                return 24000;
            case 13:
                return Camera2Constants.AudioSampleRate;
            default:
    ...
    class LookAhead {
        ...
        private boolean nextFourEquals0x64582025() throws IOException {
            return nextFourEquals(Camera2Constants.JPEG_QUALITY, (byte) 88, ClosedCaptionCtrl.RESUME_CAPTION_LOADING, ClosedCaptionCtrl.ROLL_UP_CAPTIONS_2_ROWS);
        }
        ...
        private boolean nextFourEquals0x64582025orEof() throws IOException {
            return nextFourEqualsOrEof(Camera2Constants.JPEG_QUALITY, (byte) 88, ClosedCaptionCtrl.RESUME_CAPTION_LOADING, ClosedCaptionCtrl.ROLL_UP_CAPTIONS_2_ROWS);
        }
        ...
        private boolean nextFourEqualsOrEof(byte a, byte b, byte c, byte d) throws IOException {
            if (this.buffer.limit() - this.inBufferPos >= 4) {
                if ((this.bufferStartPos + ((long) this.inBufferPos)) % PlaybackStateCompat.ACTION_SET_CAPTIONING_ENABLED == 0) {
                    System.err.println(new StringBuilder().append(((this.bufferStartPos + ((long) this.inBufferPos)) / 1024) / 1024).toString());
                }
                if (this.buffer.get(this.inBufferPos) == a && this.buffer.get(this.inBufferPos + 1) == b && this.buffer.get(this.inBufferPos + 2) == c && this.buffer.get(this.inBufferPos + 3) == d) {
                    return true;
                }
