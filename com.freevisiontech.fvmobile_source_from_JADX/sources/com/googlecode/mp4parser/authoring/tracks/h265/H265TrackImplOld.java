...
public class H265TrackImplOld {
    ...
    public int getFrameRate(ByteBuffer vps) throws IOException {
        ...
        if (vps_sub_layer_ordering_info_present_flag) {
            ...
            i2 = vps_max_sub_layers_minus1;
        }
        int[] vps_max_num_reorder_pics = new int[i2];
        if (vps_sub_layer_ordering_info_present_flag) {
            i3 = 0;
        } else {
            i3 = vps_max_sub_layers_minus1;
        }
        int[] vps_max_latency_increase_plus1 = new int[i3];
        if (vps_sub_layer_ordering_info_present_flag) {
            i4 = 0;
        } else {
            i4 = vps_max_sub_layers_minus1;
        }
        while (i4 <= vps_max_sub_layers_minus1) {
            vps_max_dec_pic_buffering_minus1[i4] = r.readUE("vps_max_dec_pic_buffering_minus1[" + i4 + "]");
            vps_max_num_reorder_pics[i4] = r.readUE("vps_max_dec_pic_buffering_minus1[" + i4 + "]");
            vps_max_latency_increase_plus1[i4] = r.readUE("vps_max_dec_pic_buffering_minus1[" + i4 + "]");
            i4++;
        }
        ...
        for (int i5 = 1; i5 <= vps_num_layer_sets_minus1; i5++) {
            for (int j = 0; j <= vps_max_layer_id; j++) {
                layer_id_included_flag[i5][j] = r.readBool("layer_id_included_flag[" + i5 + "][" + j + "]");
            }
        }
        if (r.readBool("vps_timing_info_present_flag")) {
            long readU = (long) r.readU(32, "vps_num_units_in_tick");
            long readU2 = (long) r.readU(32, "vps_time_scale");
            ...
            for (int i6 = 0; i6 < vps_num_hrd_parameters; i6++) {
                hrd_layer_set_idx[i6] = r.readUE("hrd_layer_set_idx[" + i6 + "]");
                if (i6 > 0) {
                    cprms_present_flag[i6] = r.readBool("cprms_present_flag[" + i6 + "]");
                } else {
                    cprms_present_flag[0] = true;
                }
                hrd_parameters(cprms_present_flag[i6], vps_max_sub_layers_minus1, r);
            }
        }
    ...
    public boolean isFirstOfAU(int nalUnitType, ByteBuffer nalUnit, List<ByteBuffer> accessUnit) {
        ...
        if (getNalUnitHeader(accessUnit.get(accessUnit.size() - 1)).nalUnitType <= 31) {
