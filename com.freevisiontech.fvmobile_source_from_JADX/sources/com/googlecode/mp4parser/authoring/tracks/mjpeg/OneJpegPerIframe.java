...
public class OneJpegPerIframe extends AbstractTrack {
    ...
    public OneJpegPerIframe(String name, File[] jpegs2, Track alignTo) throws IOException {
        super(name);
        this.jpegs = jpegs2;
        if (alignTo.getSyncSamples().length != jpegs2.length) {
            throw new RuntimeException("Number of sync samples doesn't match the number of stills (" + alignTo.getSyncSamples().length + " vs. " + jpegs2.length + ")");
        }
        BufferedImage a = ImageIO.read(jpegs2[0]);
        ...
        for (int i = 1; i < sampleDurationsToiAlignTo.length; i++) {
            if (currentSyncSample < syncSamples2.length && ((long) i) == syncSamples2[currentSyncSample]) {
        ...
        this.syncSamples = new long[jpegs2.length];
