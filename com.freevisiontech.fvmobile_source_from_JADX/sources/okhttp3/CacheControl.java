    ...
    public static CacheControl parse(Headers headers) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            while (pos < value.length()) {
                int tokenStart = pos;
                int pos2 = okhttp3.internal.http.HttpHeaders.skipUntil(value, pos, "=,;");
                String directive = value.substring(tokenStart, pos2).trim();
                if (pos2 == value.length() || value.charAt(pos2) == ',' || value.charAt(pos2) == ';') {
                    pos = pos2 + 1;
                    parameter = null;
                } else {
                    int pos3 = okhttp3.internal.http.HttpHeaders.skipWhitespace(value, pos2 + 1);
                    if (pos3 >= value.length() || value.charAt(pos3) != '\"') {
                        int parameterStart = pos3;
                        pos = okhttp3.internal.http.HttpHeaders.skipUntil(value, pos3, ",;");
                        parameter = value.substring(parameterStart, pos).trim();
                    } else {
                        int pos4 = pos3 + 1;
                        int parameterStart2 = pos4;
                        int pos5 = okhttp3.internal.http.HttpHeaders.skipUntil(value, pos4, "\"");
                ...
                if ("no-cache".equalsIgnoreCase(directive)) {
                    ...
                    maxAgeSeconds2 = okhttp3.internal.http.HttpHeaders.parseSeconds(parameter, -1);
                } else if ("s-maxage".equalsIgnoreCase(directive)) {
                    sMaxAgeSeconds2 = okhttp3.internal.http.HttpHeaders.parseSeconds(parameter, -1);
                    ...
                    maxStaleSeconds2 = okhttp3.internal.http.HttpHeaders.parseSeconds(parameter, Integer.MAX_VALUE);
                } else if ("min-fresh".equalsIgnoreCase(directive)) {
                    minFreshSeconds2 = okhttp3.internal.http.HttpHeaders.parseSeconds(parameter, -1);
