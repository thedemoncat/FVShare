...
public final class Cookie {
    ...
    static Cookie parse(long currentTimeMillis, HttpUrl url, String setCookie) {
        ...
        while (pos < limit) {
            ...
            } else if (attributeName.equalsIgnoreCase(SerializableCookie.DOMAIN)) {
                try {
    ...
    private static long parseMaxAge(String s) {
        ...
        try {
            ...
            if (s.matches("-?\\d+")) {
                if (!s.startsWith(Constants.FILENAME_SEQUENCE_SEPARATOR)) {
    ...
    public static List<Cookie> parseAll(HttpUrl url, Headers headers) {
        List<String> cookieStrings = headers.values(HttpHeaders.HEAD_KEY_SET_COOKIE);
