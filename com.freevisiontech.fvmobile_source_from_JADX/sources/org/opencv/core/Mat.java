...
public class Mat {
    ...
    public Mat(int rows, int cols, int type, Scalar s) {
        this.nativeObj = n_Mat(rows, cols, type, s.val[0], s.val[1], s.val[2], s.val[3]);
    }

    public Mat(Size size, int type, Scalar s) {
        this.nativeObj = n_Mat(size.width, size.height, type, s.val[0], s.val[1], s.val[2], s.val[3]);
    }
    ...
    public Mat(Mat m, Rect roi) {
        this.nativeObj = n_Mat(m.nativeObj, roi.f1131y, roi.f1131y + roi.height, roi.f1130x, roi.f1130x + roi.width);
    }
    ...
    public void locateROI(Size wholeSize, Point ofs) {
        ...
        if (ofs != null) {
            ofs.f1125x = ofs_out[0];
            ofs.f1126y = ofs_out[1];
        }
    }
    ...
    public Mat setTo(Scalar value, Mat mask) {
        return new Mat(n_setTo(this.nativeObj, value.val[0], value.val[1], value.val[2], value.val[3], mask.nativeObj));
    }
    ...
    public Mat submat(Rect roi) {
        return new Mat(n_submat(this.nativeObj, roi.f1130x, roi.f1131y, roi.width, roi.height));
    }
    ...
    public String toString() {
        return "Mat [ " + rows() + Marker.ANY_MARKER + cols() + Marker.ANY_MARKER + CvType.typeToString(type()) + ", isCont=" + isContinuous() + ", isSubmat=" + isSubmatrix() + ", nativeObj=0x" + Long.toHexString(this.nativeObj) + ", dataAddr=0x" + Long.toHexString(dataAddr()) + " ]";
    }
