...
/* renamed from: tv.danmaku.ijk.media.player.IjkMediaMeta */
public class IjkMediaMeta {
    ...
    public String getDurationInline() {
        long secs = (this.mDurationUS + 5000) / C1907C.MICROS_PER_SECOND;
        long mins = secs / 60;
        return String.format(Locale.US, "%02d:%02d:%02d", new Object[]{Long.valueOf(mins / 60), Long.valueOf(mins % 60), Long.valueOf(secs % 60)});
    }
    ...
    public static IjkMediaMeta parse(Bundle mediaMeta) {
        ...
        int i = meta.getInt("timedtext", -1);
        ArrayList<Bundle> streams = meta.getParcelableArrayList(IJKM_KEY_STREAMS);
        if (streams == null) {
            return meta;
        }
        int index = -1;
        Iterator<Bundle> it = streams.iterator();
        while (it.hasNext()) {
            Bundle streamBundle = it.next();
            index++;
    ...
    /* renamed from: tv.danmaku.ijk.media.player.IjkMediaMeta$IjkStreamMeta */
